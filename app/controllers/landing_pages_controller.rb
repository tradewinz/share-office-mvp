    ...
    if (@search_string != "")
      ...
      if(@sort == "rent")
        if(@direction == "asc")
          ...
          @offices = Office.near(@search_string, 20, :order => 'rent IS NULL, rent IS 0, rent ASC').page(params[:page])
        else
          ...
          @offices = Office.near(@search_string, 20, :order => 'rent IS NULL, rent IS 0, rent DESC').page(params[:page])
        end
      else
      ...
      if(@sort == "rent")
        if(@direction == "asc")
          @offices =  Office.order('rent IS NULL, rent IS 0, rent ASC').page(params[:page])
        else
          @offices =  Office.order('rent IS NULL, rent IS 0, rent DESC').page(params[:page])
        end
      else
